...
def readPointsScalar(filename='points_precursor',header=0,data_points=data_points_org):
    # read in the points_precursor data
    data = pd.read_csv(filename, header=header , names=['raw'])
    data['raw'] = data['raw'].map(lambda x: x.strip('()'))
...
def writeU(data_points=data_points_new,pfy=None,pfz=None,y=None,z=None):
    ...
    for id in range(idStart,idEnd):
        ...
        pfUx = Ux #griddata((y, z), Ux, (pfy, pfz), method='nearest')
        pfUy = Uy #griddata((y, z), Uy, (pfy, pfz), method='nearest')
        pfUz = Uz #griddata((y, z), Uz, (pfy, pfz), method='nearest')
...
def writeScalar(data_points=data_points_new,scalar = 'CH4',pfy=None,pfz=None,y=None,z=None):
    ...
    for id in range(idStart,idEnd):
        ...
        try:
            Scalar = readPointsScalar(filename=Scalar_path,header=1,data_points=data_points_org)
        except IndexError:
            print('Check the header lines in readPoints of writeScalar')

        # grid interpolation
        # nur z und y weil x eh x=0..
        pfScalar = Scalar #griddata((y,z), Scalar, (pfy, pfz), method='nearest')
...
x, y, z = readPointsVector('points_precursor', header=0, data_points=data_points_org)

# read in the patch field
pfx,pfy, pfz = readPointsVector('points_final', header=0, data_points=data_points_new)
